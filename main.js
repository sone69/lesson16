...
let wordsList = (str, subStr) => {
  let arr = str
    ...
    .replaceAll('"', '')
    .split(' ')
    .filter((uniqueArr) => uniqueArr.indexOf(subStr) > -1);
  let newStr = new Set();
  arr.forEach((uniqueArr) => {
    newStr.add(uniqueArr);
  });
  return newStr;
};
...
let getLocalDate = (date, isSeconds, isISO) => {
  let time;
  if (!isISO) {
    time = isSeconds ? date.toLocaleString() : date.toLocaleString().slice(0, -3);
  } else {
    ...
    time = isSeconds ? `${year}-${month}-${day}, ${hour}:${minutes}:${seconds}` : `${year}-${month}-${day}, ${hour}:${minutes}`;
  }
  return time;
};
...
let getLocalDay = (date) => {
  const numberDay = new Date(date);
  let day = numberDay.getDay();
  if (day === 0) day = 7;
  return day;
};
...
let getDateAgo = (date, days) => {
  ...
  return dateAgo.toLocaleString().slice(0, -10)
};
...
let car = new Car(2, 'Leon', 'Seat', 2008);
let car2 = new Car(2000, 'Lacetti', 'Chevrolet', 2010);
...
let testPerformance = (iterations, func) => {
  let time = Date.now();
  if (typeof func == 'function') for (iterations; iterations--;)func();
